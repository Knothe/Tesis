...
public class MarchingCell : Cells
{
    ...
    public void SetFirstValues(int3 p, float3 point, float s)
    {
        float3 noisePoint;
        position = point;
    ...
    public void SetValues(int3 p, float3 point, float s)
    {
        position = point;
        p = (p.y * axis.c2) + (p.x * axis.c0) + (p.z * axis.c1);
        for (int i = 0; i < 8; i++)
...
public class DualCell : Cells
{
    ...
    {
        SetValues(p, new float3(x, y, z), s);
    }

    public float3 GetPointNormal(float3 p, float yScale)
    {
